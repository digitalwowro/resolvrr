import { ProviderError, type ProviderContext, type TicketListQuery } from "@/core/providers";
...
function orderedAssetRecords<T>(records: Record<string, T> | undefined, recordIds: Array<string | number> | undefined): T[] {
  if (!records) {
    return [];
  }
...
function nextTicketListCursor(page: number, limit: number, loadedCount: number, totalCount: number | undefined) {
...
function collectNamedAssetIds(tickets: ZammadTicket[], idField: "state_id" | "priority_id") {
  return [
    ...new Set(
      tickets
...
function namedAssetMap(
  ...
  return Object.fromEntries(
    assets
      ...
      .filter((entry): entry is [string, ZammadGenericNamedAsset] => Boolean(entry[0])),
  );
}
