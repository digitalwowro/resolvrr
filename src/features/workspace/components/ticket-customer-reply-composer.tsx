"use client";

import { Send } from "lucide-react";
import { useState } from "react";
import { Button } from "@/components/ui";
import type {
  TicketCommunicationCapabilities,
  TicketCustomerReplyActionState,
  TicketCustomerReplyPayload,
} from "@/features/tickets/communication-model";

type TicketCustomerReplyComposerProps = {
  addTicketCustomerReplyAction(
    request: TicketCustomerReplyPayload,
  ): Promise<TicketCustomerReplyActionState>;
  communicationCapabilities: TicketCommunicationCapabilities;
  onReplySaved(ticketId: string): void;
  ticketExternalId: string;
};

function replyStatusText(
  sending: boolean,
  result: TicketCustomerReplyActionState,
) {
  if (sending) {
    return "Sending reply...";
  }
  if (result.status === "failed" || result.status === "saved-refresh-failed") {
    return result.message;
  }
  if (result.status === "saved") {
    return result.message;
  }
  return undefined;
}

export function TicketCustomerReplyComposer({
  addTicketCustomerReplyAction,
  communicationCapabilities,
  onReplySaved,
  ticketExternalId,
}: TicketCustomerReplyComposerProps) {
  const [body, setBody] = useState("");
  const [sending, setSending] = useState(false);
  const [result, setResult] = useState<TicketCustomerReplyActionState>({
    status: "idle",
  });
  const statusText = replyStatusText(sending, result);
  const canSubmit =
    communicationCapabilities.customerReplies &&
    body.trim().length > 0 &&
    !sending;

  function submitReply() {
    if (!canSubmit) {
      return;
    }

    setSending(true);
    setResult({ status: "idle" });
    void addTicketCustomerReplyAction({ body, ticketExternalId })
      .then((nextResult) => {
        setResult(nextResult);
        if (
          nextResult.status === "saved" ||
          nextResult.status === "saved-refresh-failed"
        ) {
          setBody("");
        }
        if (nextResult.status === "saved") {
          onReplySaved(ticketExternalId);
        }
      })
      .catch(() =>
        setResult({
          status: "failed",
          message: "The customer reply could not be sent. Try again.",
        }),
      )
      .finally(() => setSending(false));
  }

  if (!communicationCapabilities.customerReplies) {
    return null;
  }

  return (
    <form
      className="shrink-0 border-t border-slate-200 bg-white px-4 py-3"
      onSubmit={(event) => {
        event.preventDefault();
        submitReply();
      }}
    >
      <label
        className="mb-1 block text-xs font-semibold text-slate-700"
        htmlFor={`customer-reply-${ticketExternalId}`}
      >
        Customer reply
      </label>
      <div className="flex items-end gap-2">
        <textarea
          className="min-h-20 flex-1 resize-y rounded-md border border-slate-300 px-3 py-2 text-sm leading-5 text-slate-900 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
          disabled={sending}
          id={`customer-reply-${ticketExternalId}`}
          onChange={(event) => {
            setBody(event.target.value);
            setResult({ status: "idle" });
          }}
          value={body}
        />
        <Button className="shrink-0" disabled={!canSubmit} type="submit">
          <Send aria-hidden="true" className="size-4" />
          Send reply
        </Button>
      </div>
      {statusText ? (
        <p
          className={
            result.status === "failed" || result.status === "saved-refresh-failed"
              ? "mt-2 text-xs text-amber-700"
              : "mt-2 text-xs text-slate-600"
          }
          role={sending || result.status === "saved" ? "status" : "alert"}
        >
          {statusText}
        </p>
      ) : null}
    </form>
  );
}
