...
advertises `ticket:list`, `ticket:sort`, `ticket:detail`,
...
Zammad sort fields. Zammad still does not advertise provider-backed count,
grouping, grouped count, or full-text search yet, so those requests are rejected
by the provider-neutral guardrail layer before Zammad list code is called.
