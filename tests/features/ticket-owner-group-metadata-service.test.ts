...
describe("ticket owner and group metadata mutation service", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("updates owner and group metadata through separate provider capabilities", async () => {
    ...
    const result = await updateWorkspaceTicketMetadata(
      ...
      "user-1",
      "ticket-1",
      { ownerExternalId: "agent-2", groupExternalId: "group-2" },
    );
  ...
  it("rejects owner or group updates when their capability is unavailable", async () => {
    const updateTicketMetadata = vi.fn().mockResolvedValue(undefined);
    ...
    const result = await updateWorkspaceTicketMetadata(
      ...
      createProviderRegistry([
        provider({
          capabilities: ["ticket:list", "ticket:detail", "ticket:update-owner"],
          updateTicketMetadata,
        }),
      ]),
      encryptionKey,
